	...
	match current_state:
		states.IDLE:
			...
			(Input.is_action_just_pressed("walk_right") and not is_on_right_wall)):
				return states.RUN
			elif (Input.is_action_just_pressed("walk_left") and not is_on_left_wall) or\
			(Input.is_action_just_pressed("walk_right") and not is_on_right_wall):
				return states.WALK
			elif Input.is_action_just_pressed("place_simple_tool"):
		...
		states.WALK:
			if Input.is_action_just_pressed("jump"):
				...
				return states.FALL
			elif not (Input.is_action_pressed("walk_left") or Input.is_action_just_pressed("walk_right")):
				return states.IDLE
			return states.WALK
		states.RUN:
			if (Input.is_action_pressed("walk_right") and is_on_right_wall) or \
			(Input.is_action_pressed("walk_left") and is_on_left_wall):
				return states.IDLE
		...
		states.PLACE_FLOOR_TOOL:
			if Input.is_action_just_released("place_simple_tool"):
				if Input.is_action_pressed("walk_left") or Input.is_action_pressed("walk_right"):
					return states.WALK
				else:
					return states.IDLE
