...
func _on_area_2d_area_entered(area: Area2D) -> void:
	if area.name == "Floortool_placement_zone":
		...
		var fly_rotation = randi_range(-25,-60)
		play("default")
		if area.global_position.x < global_position.x:
			flip_h = true
			fly_rotation = -fly_rotation
		get_tree().create_tween().tween_property($".", "position", Vector2.UP.rotated(deg_to_rad(fly_rotation)) * 30000, 5).set_trans(Tween.TRANS_CUBIC)
